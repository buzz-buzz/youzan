      ...
      const fn = async () => {
        ...
        await bluebird.each(data, async ({ env, mobile, period, num, pic_path, title, buyer_messages }) => {
          ...
          if (getUserErr) {
            doc.ongoing = false
            await doc.save()
            logger.debug('youzan:getUserByWechat:error', {
              url: `${config.get('api.buzzService')}/api/v1/users/by-wechat?openid=${openid}`,
              res: resUserByWechat,
              err: getUserErr,
            })
...
**错误**: ${getUserErr}`,
          ...
          const userByWechat = _.get(resUserByWechat, 'data.user_id')
          ...
          if (!userId) {
            ...
            if (createUserErr) {
              doc.ongoing = false
              await doc.save()
              logger.debug('youzan:createUser:error', {
                url: `${config.get('api.buzzService')}/api/v1/users`,
                data: newUserData,
                res: newUserRes,
                err: createUserErr,
...
${buyer_message && `**备注**: ${buyer_message}`}`.replace(/\n$/g, '')
          if (addClassHoursErr) {
            logger.debug('youzan:addClassHours:error', {
              url: `${config.get('api.buzzService')}/api/v1/user-balance/${userId}`,
              data: {
                class_hours: classHours,
              },
