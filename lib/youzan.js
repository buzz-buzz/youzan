...
const Yz = {
  ...
  async getTradeByTradeId(tid) {
    const { trade } = await Yz.api('trade.get', { tid, fields: 'tid,status,orders,title,created,update_time,fans_info' })
  ...
  async getRecentTrades(hour) {
    ...
    const opts = {
      ...
      fields: 'tid,status,orders,title,created,update_time,fans_info',
    }
  ...
  async parseTrades(trade, opt) {
    ...
    await bluebird.each(trades, async ({ tid, status, orders, title, created, update_time, fans_info }) => {
      const fn = async () => {
        ...
        const openid = _.get(fans_info, 'fans_weixin_openid')
        const data = []
        ...
        await bluebird.each(data, async ({ env, mobile, period, num }) => {
          // TODO: 给 env 环境的 openid 的用户添加 num * period 课时
          logger.debug('youzan:trade:parse:add', { env, openid, add: num * period })
          // await axios({
          //   url: `${config.get('api.buzzService')}/user/`
          // })
        })
        doc.added = true
        await doc.save()
      ...
      await fn().catch(e => {
        logger.error('youzan:trade:parse:error', e, { tid, status, orders, title, created, update_time, fans_info })
      })
    })
  },
  parseMsg(v) {
