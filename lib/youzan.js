    ...
    await bluebird.each(trades, async trade => {
      ...
      const fn = async () => {
        ...
        await bluebird.each(data, async ({ env, mobile, period, num, pic_path, title, buyer_messages }) => {
          ...
          logger.debug('youzan:trade:parse:classHours', { env, openid, classHours, pic_path, title, mobile, buyer_message, buyer_messages })
          let getUserErr
          const { data: userByWechat } = await axios({
            url: `${config.get('api.buzzService')}/api/v1/users/by-wechat?openid=${openid}`,
          }).catch(e => {
          ...
          if (getUserErr) {
            return await axios({
              ...
              method: 'post',
              data: {
                channel: config.get('notify.youzan.channel'),
                text: `**订单**: [${tid}](https://www.youzan.com/v2/trade/order/detail?order_no=${tid})
**名称**: ${title}
...
**错误**: ${getUserErr}`,
          ...
          let beforeClassHours = _.get(userByWechat, 'class_hours', 0)
          let createUserErr
          if (!userId) {
            ...
            const { data: newUser } = await axios({
              method: 'post',
              url: `${config.get('api.buzzService')}/api/v1/users`,
              data: newUserData,
            }).catch(e => {
  ...
  **错误**: ${createUserErr}`,
          ...
          const { data: { class_hours: currentClassHours } } = await axios({
            method: 'put',
            url: `${config.get('api.buzzService')}/api/v1/user-balance/${userId}`,
            data: {
...
**有赞用户**: [${nickname}](https://www.youzan.com/scrm/customer/customer/#/info/${buyerId})
**后台用户**: [${nickname}](http://admin-test.buzzbuzzenglish.com/students/${userId})
${buyer_messages && `${_.chain(buyer_messages)
    .filter(i => _.size(i.content) > 0)
    .map(i => {
      if (i.title === '手机号') {
        return `**${i.title}**: [${i.content}](tel://${i.content})`
      }
